...
def main():
    ...
    if is_streamlit:
        ...
        st.set_page_config(
            page_title="반도체 수출 실적 대시보드 (실습 3)",
            page_icon="📊",
            layout="wide",
        ...
        st.markdown("<div class='sub-title'>미국 및 베트남 대상 수출금액 상위 10건 내역을 분석하고 시각화합니다. (day04_3.py 실행 중)</div>", unsafe_allow_html=True)
        ...
        try:
            ...
            st.success(f"✅ 공식 필터링 가이드에 따른 상위 10건 데이터가 로컬 파일 `{standard_report_path}`로 자동 업데이트 및 저장되었습니다.")
        ...
        # --- [터미널 모드 (일반 실행)] ---
        # Streamlit 명령어(streamlit run)가 아닌 일반 파이썬 명령어(python day04_3.py)로 실행했을 때 동작하는 부분입니다.
        print("=" * 60)
