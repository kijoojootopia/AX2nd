...
# streamlit run 02.py   실행하는 방법 터미널 cmd 입력

# st.title("내용") 은 페이지에서 가장 크고 굵은 제목을 만든다.(h1 느낌)
st.title("무역 데이터 부트캠프 자기소개")

# st.header('내용')은 title보다 한 단계 작은 큰 제목(h2 느낌)
st.header('유진이 바보🐈‍⬛: Streamlit으로 만든 첫 페이지 입니다.')

# st.subheader('내용')은 header보다 한 단계 작은 제목(h3 느낌)
st.subheader('오늘 배운 것: 텍스트를 화면에 예쁘게 보여주는 방법')

# st.text('내용') 꾸밈이 전혀 없는 순수 텍스트를 그대로 출력
st.text('st.txt로 출력한 문장입니다, 줄을 바꾸거나 굵게 등의 서식이 적용되지 않는다.')
st.text('😒')

# st.captioon('내용') 아주 작은 글씨로 보조 설명을 넣을 떄
st.caption('파이썬 강의보다 즐겁군')
